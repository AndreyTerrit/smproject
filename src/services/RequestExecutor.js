...
class RequestExecutor {
     ...
     * @return {Promise}
     */
    //Вызывается только когда выполняется функция, а выпоняется она тлько когда мы открываем первую вьюшку, следовательно обеспечена бесконченая загрузка на втором view при обновлении страницы
    async execute(url, exact, init, data) {
        if(this.loadingMask) store.commit("setIsLoading", true);

        try {
            if(data) init = {...init, body: JSON.stringify(data)};//Если пут или пост запрос то init

            const location = exact ? url : this.baseUrl + url;
            const response = await fetch(location, init);//init - заголовок
